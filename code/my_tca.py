...
def get_kernel(kerneltype, kernelparam, x1, x2=None):
    '''
...
def tca(x_src, x_tar, x_tar_o=None, dim=5, kerneltype='rbf', kernelparam=1, mu=1, lambdaparam=0):
    '''
    ...
    :param mu: param
    :param lambdaparam: lambda param,default is 0. If not 0, then implemented Deform, which is on the process.
    ...
    L = get_L(n_src, n_tar)
    L[np.isnan(L)] = 0
    K = get_kernel(kerneltype, kernelparam, X)
    K[np.isnan(K)] = 0
    if x_tar_o is not None:
        K_tar_o = get_kernel(kerneltype, kernelparam, X, x_tar_o)
    ...
    forPinv = mu * np.identity(n_src + n_tar) + np.dot(np.dot(K, L), K)
    ...
    x_src_tca = np.asarray(x_src_tca[:, :dim], dtype=float)
    x_tar_tca = np.asarray(x_tar_tca[:, :dim], dtype=float)
    if x_tar_o is not None:
        x_tar_o_tca = x_tar_o_tca[:, :dim]
...
if __name__ == '__main__':
    ...
    x_src = data[:, :81]
    x_tar = data[:, 81:]
    x_src_tca, x_tar_tca, x_tar_o_tca = tca(x_src, x_tar, x_tar_o=None, dim=30, lambdaparam=0)
    np.savetxt('x_src.csv', x_src_tca, delimiter=',', fmt='%.6f')
    np.savetxt('x_tar.csv', x_tar_tca, delimiter=',', fmt='%.6f')
